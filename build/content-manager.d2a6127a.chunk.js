...
const CustomDragLayer = () => {
  const { itemType, isDragging, item, initialOffset, currentOffset, mouseOffset } = (0,cjs.useDragLayer)((monitor) => ({
...
const useContentTypeLayout = () => {
  const currentLayout = (0,lib/* useSelector */.v9)(selectors);
  const getComponentLayout = (0,react.useCallback)((componentUid) => {
...
const formatLayoutToApi = (_a) => {
  ...
  const edit = layouts.edit.map((row) => row.map(({ name, size }) => ({
    name,
    size
  })));
...
const getMaxTempKey = (arr) => {
  ...
  return Math.max.apply(Math, arr.map((o) => o.__temp_key__));
};
  ...
  return Object.keys(obj).reduce((acc, current) => {
    ...
    if (typeof value === "object") {
      if (value._isAMomentObject === true) {
        return removeKeyInObject_spreadProps(removeKeyInObject_spreadValues({}, acc), { [current]: value });
      }
      if (Array.isArray(acc)) {
        acc[current] = removeKeyInObject(value, keyToRemove);
        return acc;
    ...
    return Object.keys(data2).reduce((acc, current) => {
      ...
      if (attrType === "dynamiczone") {
        acc[current] = value.map((componentValue) => {
          const subCleanedData = recursiveCleanData(componentValue, componentSchema[componentValue.__component]);
...
const formatEditRelationsLayoutWithMetas = (contentTypeConfiguration, models) => {
  const formatted = contentTypeConfiguration.layouts.editRelations.reduce((acc, current) => {
    ...
    const targetModelUID = (0,lodash.get)(contentTypeConfiguration, ["attributes", current, "targetModel"], null);
...
const formatLayouts = (initialData, models) => {
  ...
  Object.keys(data.components).forEach((compoUID) => {
    const formattedCompoEditLayout = formatLayoutWithMetas(data.components[compoUID], ctUid, models);
  ...
  const formatted = contentTypeConfiguration.layouts.edit.reduce((acc, current) => {
    const row = current.map((attribute) => {
      ...
      if (fieldSchema.type === "relation") {
        ...
        const targetModelSchema = getRelationModel(targetModelUID, models);
        const targetModelPluginOptions = targetModelSchema.pluginOptions || {};
        const queryInfos = ctUid ? generateRelationQueryInfosForComponents(contentTypeConfiguration, attribute.name, ctUid, models) : generateRelationQueryInfos(contentTypeConfiguration, attribute.name, models);
...
const generateRelationQueryInfos = (contentTypeConfiguration, fieldName, models) => {
  ...
  const mainField = (0,lodash.get)(contentTypeConfiguration, ["metadatas", fieldName, "edit", "mainField", "name"], "");
...
var App_reducer = __webpack_require__(2006);
...
const useFetchContentTypeLayout = (contentTypeUID) => {
  ...
  const { schemas } = (0,lib/* useSelector */.v9)((state) => schemasSelector(state), lib/* shallowEqual */.wU);
  const isMounted = (0,react.useRef)(true);
  const getData = (0,react.useCallback)((uid, source) => __async(undefined, null, function* () {
    if (layouts[uid]) {
  ...
  const updateLayout = (0,react.useCallback)((newLayout) => {
    dispatch({
      type: "UPDATE_LAYOUT",
      newLayout: utils_formatLayouts({ contentType: newLayout, components: {} }, schemas)
    });
  }, [schemas]);
  return {
    error,
...
const useFindRedirectionLink = (slug) => {
  ...
  const collectionTypesMenuLinks = (0,lib/* useSelector */.v9)(useFindRedirectionLink_selectors);
...
var constants = __webpack_require__(22816);
...
const useSyncRbac = (query, collectionTypeUID, containerName = "listView") => {
  const collectionTypesRelatedPermissions = (0,lib/* useSelector */.v9)(selectCollectionTypePermissions);
  const permissions = (0,lib/* useSelector */.v9)(selectPermissions);
...
var exposedHooks = __webpack_require__(17907);
// EXTERNAL MODULE: ./.cache/admin/src/content-manager/pages/EditViewLayoutManager/constants.js
var EditViewLayoutManager_constants = __webpack_require__(48669);
...
var contexts = __webpack_require__(40448);
...
function useSelect(name) {
  ...
  const dynamicDisplayedComponents = (0,react.useMemo)(() => (0,lodash.get)(modifiedData, [name], []).map((data) => data.__component), [modifiedData, name]);
...
const loadCss = () => mdRenderer_async(undefined, null, function* () {
...
const Hint = ({ id, error, name, description }) => {
  const { formatMessage } = (0,react_intl.useIntl)();
  const hint = description ? formatMessage({ id: description.id, defaultMessage: description.defaultMessage }, Hint_spreadValues({}, description.values)) : "";
  if (!hint || error) {
    return null;
  }
...
const listHandler = (editor, listType) => {
  ...
  if (doc.somethingSelected()) {
    ...
    editor.current.setSelection({ line: currentLine, ch: 0 }, { line: currentLine, ch: lineContent.length });
...
const Wysiwyg_Wysiwyg = ({
  ...
  const formattedPlaceholder = placeholder ? formatMessage({ id: placeholder.id, defaultMessage: placeholder.defaultMessage }, Wysiwyg_spreadValues({}, placeholder.values)) : "";
  const label = intlLabel.id ? formatMessage({ id: intlLabel.id, defaultMessage: intlLabel.defaultMessage }, Wysiwyg_spreadValues({}, intlLabel.values)) : name;
...
var jsonlint = __webpack_require__(19257);
...
const Label = ({ intlLabel, labelAction, name, required }) => {
  const { formatMessage } = (0,react_intl.useIntl)();
  const label = (intlLabel == null ? void 0 : intlLabel.id) ? formatMessage({ id: intlLabel.id, defaultMessage: intlLabel.defaultMessage }, Label_spreadValues({}, intlLabel.values)) : name;
...
const InputJSON_loadCss = () => InputJSON_async(undefined, null, function* () {
...
class InputJSON extends react.Component {
  constructor(props) {
    ...
    __publicField(this, "markSelection", ({ message }) => {
      ...
      const markedText = this.codeMirror.markText({ line, ch: chStart }, { line, ch: chEnd }, { className: "colored" });
...
const InputUID = ({
  ...
  const label = intlLabel.id ? formatMessage({ id: intlLabel.id, defaultMessage: intlLabel.defaultMessage }, InputUID_spreadValues({}, intlLabel.values)) : name;
  const hint = description ? formatMessage({ id: description.id, defaultMessage: description.defaultMessage }, InputUID_spreadValues({}, description.values)) : "";
  const formattedPlaceholder = placeholder ? formatMessage({ id: placeholder.id, defaultMessage: placeholder.defaultMessage }, InputUID_spreadValues({}, placeholder.values)) : "";
  generateUid.current = (shouldSetInitialValue = false) => InputUID_async(undefined, null, function* () {
    setIsLoading(true);
  ...
  const checkAvailability = () => InputUID_async(undefined, null, function* () {
    setIsLoading(true);
  ...
  const handleGenerateMouseEnter = () => {
    setRegenerateLabel(formatMessage({
...
var design_system_Link = __webpack_require__(56156);
...
const SingleValue = (props) => {
  const { formatMessage } = (0,react_intl.useIntl)();
  const Component = react_select_cjs_prod/* components.SingleValue */.wx.SingleValue;
...
function SelectOne({
  ...
  return /* @__PURE__ */ react.createElement(Stack.Stack, {
    spacing: 1
  }, /* @__PURE__ */ react.createElement(react_select_cjs_prod/* default */.ZP, {
    ...
    onMenuOpen,
    onMenuScrollToBottom,
    placeholder: formatMessage(placeholder || { id: "global.select", defaultMessage: "Select..." }),
    styles,
...
function ListItem({
  ...
  return /* @__PURE__ */ react.createElement(Flex.Flex, {
    ...
    isDraft,
    title
  })), displayNavigationLink ? /* @__PURE__ */ react.createElement(design_system_Link.Link, {
...
function SelectMany({
  ...
  return /* @__PURE__ */ react.createElement(Stack.Stack, {
    ...
    filterOption: (candidate, input) => {
      ...
      if (input) {
        return (0,react_select_cjs_prod/* createFilter */.cj)(filterConfig)(candidate, input);
      }
      return true;
    },
    mainField,
...
;// CONCATENATED MODULE: ./.cache/admin/src/content-manager/components/SelectWrapper/IconBox.js
...
`;
/* harmony default export */ const SelectWrapper_IconBox = (IconBox);

;// CONCATENATED MODULE: ./.cache/admin/src/content-manager/components/SelectWrapper/ClearIndicator.js
var ClearIndicator_defProp = Object.defineProperty;
var ClearIndicator_getOwnPropSymbols = Object.getOwnPropertySymbols;
var ClearIndicator_hasOwnProp = Object.prototype.hasOwnProperty;
var ClearIndicator_propIsEnum = Object.prototype.propertyIsEnumerable;
var ClearIndicator_defNormalProp = (obj, key, value) => key in obj ? ClearIndicator_defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var ClearIndicator_spreadValues = (a, b) => {
  for (var prop in b || (b = {}))
    if (ClearIndicator_hasOwnProp.call(b, prop))
      ClearIndicator_defNormalProp(a, prop, b[prop]);
  if (ClearIndicator_getOwnPropSymbols)
    for (var prop of ClearIndicator_getOwnPropSymbols(b)) {
      if (ClearIndicator_propIsEnum.call(b, prop))
        ClearIndicator_defNormalProp(a, prop, b[prop]);
    }
  return a;
};
...
const ClearIndicator = (props) => {
  const Component = react_select_cjs_prod/* components.ClearIndicator */.wx.ClearIndicator;
  return /* @__PURE__ */ react.createElement(Component, ClearIndicator_spreadValues({}, props), /* @__PURE__ */ react.createElement(SelectWrapper_IconBox, {
    as: "button",
    type: "button"
  }, /* @__PURE__ */ react.createElement((Cross_default()), null)));
};
/* harmony default export */ const SelectWrapper_ClearIndicator = (ClearIndicator);

;// CONCATENATED MODULE: ./.cache/admin/src/content-manager/components/SelectWrapper/DropdownIndicator.js




const CarretBox = (0,styled_components_browser_cjs["default"])(SelectWrapper_IconBox)`
  display: flex;
  background: none;
  border: none;
...
const DropdownIndicator = () => {
  return /* @__PURE__ */ react.createElement(CarretBox, {
    as: "button",
    type: "button",
    paddingRight: 3
  }, /* @__PURE__ */ react.createElement((CarretDown_default()), null));
};
/* harmony default export */ const SelectWrapper_DropdownIndicator = (DropdownIndicator);

;// CONCATENATED MODULE: ./.cache/admin/src/content-manager/components/SelectWrapper/IndicatorSeparator.js
const IndicatorSeparator = () => null;
/* harmony default export */ const SelectWrapper_IndicatorSeparator = (IndicatorSeparator);
...
const Option = (props) => {
  const { formatMessage } = (0,react_intl.useIntl)();
  const Component = react_select_cjs_prod/* components.Option */.wx.Option;
...
;// CONCATENATED MODULE: ./.cache/admin/src/content-manager/components/SelectWrapper/utils/getSelectStyles.js
var getSelectStyles_defProp = Object.defineProperty;
var getSelectStyles_defProps = Object.defineProperties;
var getSelectStyles_getOwnPropDescs = Object.getOwnPropertyDescriptors;
var getSelectStyles_getOwnPropSymbols = Object.getOwnPropertySymbols;
var getSelectStyles_hasOwnProp = Object.prototype.hasOwnProperty;
var getSelectStyles_propIsEnum = Object.prototype.propertyIsEnumerable;
var getSelectStyles_defNormalProp = (obj, key, value) => key in obj ? getSelectStyles_defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var getSelectStyles_spreadValues = (a, b) => {
  for (var prop in b || (b = {}))
    if (getSelectStyles_hasOwnProp.call(b, prop))
      getSelectStyles_defNormalProp(a, prop, b[prop]);
  if (getSelectStyles_getOwnPropSymbols)
    for (var prop of getSelectStyles_getOwnPropSymbols(b)) {
      if (getSelectStyles_propIsEnum.call(b, prop))
        getSelectStyles_defNormalProp(a, prop, b[prop]);
    }
  return a;
};
var getSelectStyles_spreadProps = (a, b) => getSelectStyles_defProps(a, getSelectStyles_getOwnPropDescs(b));
const getSelectStyles = (theme) => {
  return {
    clearIndicator: (base) => getSelectStyles_spreadProps(getSelectStyles_spreadValues({}, base), { padding: 0, paddingRight: theme.spaces[3] }),
    container: (base) => getSelectStyles_spreadProps(getSelectStyles_spreadValues({}, base), {
    ...
    control: (base, state) => {
      ...
      return getSelectStyles_spreadProps(getSelectStyles_spreadValues({}, base), {
        fontSize: 14,
        height: 40,
        border,
        outline: 0,
    ...
    indicatorContainer: (base) => getSelectStyles_spreadProps(getSelectStyles_spreadValues({}, base), { padding: 0, paddingRight: theme.spaces[3] }),
    input: (base) => getSelectStyles_spreadProps(getSelectStyles_spreadValues({}, base), { margin: 0, padding: 0 }),
    menu: (base) => {
      return getSelectStyles_spreadProps(getSelectStyles_spreadValues({}, base), {
        width: "100%",
    ...
    menuList: (base) => getSelectStyles_spreadProps(getSelectStyles_spreadValues({}, base), {
    ...
    option: (base, state) => {
      ...
      return getSelectStyles_spreadProps(getSelectStyles_spreadValues({}, base), { lineHeight: theme.spaces[5], backgroundColor, borderRadius: 4 });
    },
    placeholder: (base) => getSelectStyles_spreadProps(getSelectStyles_spreadValues({}, base), { marginLeft: 0 }),
    singleValue: (base) => getSelectStyles_spreadProps(getSelectStyles_spreadValues({}, base), { marginLeft: 0, color: theme.colors.neutral800 }),
    valueContainer: (base) => getSelectStyles_spreadProps(getSelectStyles_spreadValues({}, base), {
      padding: 0,
...
/* harmony default export */ const utils_getSelectStyles = (getSelectStyles);
...
function SelectWrapper({
  ...
  const { pathname } = (0,react_router_dom_min.useLocation)();
  const theme = (0,styled_components_browser_cjs.useTheme)();
  ...
  const isSingle = ["oneWay", "oneToOne", "manyToOne", "oneToManyMorph", "oneToOneMorph"].includes(relationType);
  ...
  const getData = (0,react.useCallback)((source) => SelectWrapper_async(this, null, function* () {
    if (isMorph) {
      setIsLoading(false);
      return;
    }
    ...
    try {
      const { data } = yield utils/* axiosInstance.post */.be.post(endPoint, { idsToOmit }, { params, cancelToken: source.token });
      const formattedData = data.map((obj) => {
        return { value: obj, label: obj[mainField.name] };
      });
      setOptions((prevState) => prevState.concat(formattedData).filter((obj, index) => {
  ...
  if (isSingle && value && shouldDisplayRelationLink) {
    link = /* @__PURE__ */ react.createElement(design_system_Link.Link, {
      to: { pathname: to, state: { from: pathname }, search: searchToPersist }
  ...
  const styles = utils_getSelectStyles(theme);
  return /* @__PURE__ */ react.createElement(Stack.Stack, {
    spacing: 1
    ...
    components: {
      ClearIndicator: SelectWrapper_ClearIndicator,
      DropdownIndicator: SelectWrapper_DropdownIndicator,
      IndicatorSeparator: SelectWrapper_IndicatorSeparator,
...
function Inputs({
  ...
  const validations = (0,react.useMemo)(() => {
    const inputValidations = omit_default()(fieldSchema, shouldNotRunValidations ? [...VALIDATIONS_TO_OMIT, "required", "minLength"] : VALIDATIONS_TO_OMIT);
...
function getComponentErrorKeys(name, formErrors) {
  return Object.keys(formErrors).filter((errorKey) => errorKey.startsWith(name)).map((errorKey) => errorKey.split(".").slice(0, name.split(".").length + 1).join("."));
}
...
function DraggedItem_utils_select_useSelect({ schema, componentFieldName }) {
  ...
  const displayedValue = (0,lodash.toString)((0,lodash.get)(modifiedData, [...componentFieldName.split("."), mainField], ""));
  return {
    displayedValue,
    mainField,
    checkFormErrors,
...
const DraggedItem = ({
  ...
  const [, drop] = (0,cjs.useDrop)({
    ...
    hover(item, monitor) {
      ...
      const pathToComponentArray = fullPathToComponentArray.slice(0, fullPathToComponentArray.length - 1);
...
const RepeatableComponent = ({
  ...
  const doesRepComponentHasChildError = componentErrorKeys.some((error) => error.split(".").length > 1);
...
const ComponentPicker = ({ components, isOpen, onClickAddComponent }) => {
  ...
  const handleAddComponentToDz = (0,react.useCallback)((componentUid) => {
    onClickAddComponent(componentUid);
    setCategoryToOpen("");
  }, [onClickAddComponent]);
  const handleClickToggle = (0,react.useCallback)((categoryName) => {
...
const DynamicZone = ({
  ...
  const [componentCollapses, setComponentsCollapses] = (0,react.useState)(createCollapses(dynamicDisplayedComponentsLength));
  (0,react.useEffect)(() => {
  ...
  (0,react.useEffect)(() => {
    if (shouldOpenAddedComponent) {
      setComponentsCollapses((prev) => prev.map((collapse, index) => {
  ...
  const handleAddComponent = (0,react.useCallback)((componentUid) => {
    setIsOpen(false);
  ...
  const handleToggleComponent = (indexToToggle) => {
    setComponentsCollapses((prev) => prev.map(({ isOpen: isOpen2 }, index) => {
...
var crudReducer_constants = __webpack_require__(49965);
...
const CollectionTypeFormWrapper = ({ allLayoutData, children, slug, id, origin }) => {
  ...
  const {
    ...
    isLoading,
    status
  } = (0,lib/* useSelector */.v9)(crudReducer_selectors);
  ...
  const cleanClonedData = (0,react.useCallback)((data2) => {
    if (!origin) {
      return data2;
    }
    const cleaned = (0,build.contentManagementUtilRemoveFieldsFromData)(data2, allLayoutDataRef.current.contentType, allLayoutDataRef.current.components);
    return cleaned;
  }, [origin]);
  const cleanReceivedData = (0,react.useCallback)((data2) => {
    const cleaned = utils_removePasswordFieldsFromData(data2, allLayoutDataRef.current.contentType, allLayoutDataRef.current.components);
    return (0,build.formatContentTypeData)(cleaned, allLayoutDataRef.current.contentType, allLayoutDataRef.current.components);
  }, []);
  (0,react.useEffect)(() => {
    const componentsDataStructure2 = Object.keys(allLayoutData.components).reduce((acc, current) => {
      const defaultComponentForm = utils_createDefaultForm(get_default()(allLayoutData, ["components", current, "attributes"], {}), allLayoutData.components);
      acc[current] = (0,build.formatContentTypeData)(defaultComponentForm, allLayoutData.components[current], allLayoutData.components);
      return acc;
    }, {});
    const contentTypeDataStructure2 = utils_createDefaultForm(allLayoutData.contentType.attributes, allLayoutData.components);
    const contentTypeDataStructureFormatted = (0,build.formatContentTypeData)(contentTypeDataStructure2, allLayoutData.contentType, allLayoutData.components);
  ...
  (0,react.useEffect)(() => {
    const CancelToken = (axios_default()).CancelToken;
    const source = CancelToken.source();
    const fetchData = (source2) => CollectionTypeFormWrapper_async(undefined, null, function* () {
      dispatch(getData());
      try {
    ...
    const init = () => CollectionTypeFormWrapper_async(undefined, null, function* () {
  ...
  const displayErrors = (0,react.useCallback)((err) => {
  ...
  const onDelete = (0,react.useCallback)((trackerProperty) => CollectionTypeFormWrapper_async(undefined, null, function* () {
    try {
      trackUsageRef.current("willDeleteEntry", trackerProperty);
      const { data: data2 } = yield utils/* axiosInstance.delete */.be["delete"](utils_getRequestUrl(`collection-types/${slug}/${id}`));
      toggleNotification({
        type: "success",
  ...
  const onPost = (0,react.useCallback)((body, trackerProperty) => CollectionTypeFormWrapper_async(undefined, null, function* () {
  ...
  const onPublish = (0,react.useCallback)(() => CollectionTypeFormWrapper_async(undefined, null, function* () {
    try {
  ...
  const onPut = (0,react.useCallback)((body, trackerProperty) => CollectionTypeFormWrapper_async(undefined, null, function* () {
  ...
  const onUnpublish = (0,react.useCallback)(() => CollectionTypeFormWrapper_async(undefined, null, function* () {
      ...
      switch (attrType) {
        ...
        case "dynamiczone":
          cleanedData = value.map((componentData) => {
            const subCleanedData = recursiveCleanData(componentData, componentsSchema[componentData.__component]);
            return subCleanedData;
          });
          break;
        default:
...
const createYupSchema = (model, { components }, options = { isCreatingEntry: true, isDraft: true, isFromComponent: false }) => {
  ...
  return yup_lib/* object */.Ry().shape(Object.keys(attributes).reduce((acc, current) => {
    ...
    if (attribute.type === "dynamiczone") {
      let dynamicZoneSchema = yup_lib/* array */.IX().of(yup_lib/* lazy */.Vo(({ __component }) => {
        return createYupSchema(components[__component], { components }, schema_spreadProps(schema_spreadValues({}, options), { isFromComponent: true }));
      }));
...
const reducer_reducer = (state, action) => (0,immer_cjs_production_min["default"])(state, (draftState) => {
  switch (action.type) {
    case "ADD_NON_REPEATABLE_COMPONENT_TO_FIELD": {
      set_default()(draftState, ["modifiedData", ...action.keys], state.componentsDataStructure[action.componentUid]);
      break;
    }
    ...
    case "MOVE_COMPONENT_FIELD": {
      ...
      const updatedValue = utils_moveFields(currentValue, action.dragIndex, action.hoverIndex, valueToInsert);
    ...
    case "MOVE_FIELD": {
      ...
      const updatedValue = utils_moveFields(currentValue, action.dragIndex, action.overIndex, valueToInsert);
      set_default()(draftState, ["modifiedData", ...action.keys], updatedValue);
      break;
    }
    case "ON_CHANGE": {
...
;// CONCATENATED MODULE: ./.cache/admin/src/content-manager/components/EditViewDataManagerProvider/utils/getAPIInnerError.js

function getAPIInnerError(error) {
  const errorPayload = error.response.data.error.details.errors;
  const validationErrors = errorPayload.reduce((acc, err) => {
    acc[err.path.join(".")] = {
      id: utils_getTrad(`apiError.${err.message}`),
...
const EditViewDataManagerProvider = ({
  ...
  const addRepeatableComponentToField = (0,react.useCallback)((keys, componentUid, shouldCheckErrors2 = false) => {
    dispatch({
  ...
  const yupSchema = (0,react.useMemo)(() => {
    ...
    return utils_schema(currentContentTypeLayout, {
  ...
  const checkFormErrors = (0,react.useCallback)((..._0) => EditViewDataManagerProvider_async(undefined, [..._0], function* (dataToSet = {}) {
    let errors = {};
  ...
  const handleChange = (0,react.useCallback)(({ target: { name, value, type } }, shouldSetInitialValue = false) => {
  ...
  const createFormData = (0,react.useCallback)((data) => {
    const preparedData = utils_removeKeyInObject(cloneDeep_default()(data), "__temp_key__");
    const cleanedData = utils_cleanData(preparedData, currentContentTypeLayout, allLayoutData.components);
    return cleanedData;
  ...
  const handleSubmit = (0,react.useCallback)((e) => EditViewDataManagerProvider_async(undefined, null, function* () {
    e.preventDefault();
    let errors = {};
    try {
    ...
    try {
      ...
      errors = EditViewDataManagerProvider_spreadValues(EditViewDataManagerProvider_spreadValues({}, errors), getAPIInnerError(err));
    }
    dispatch({
      type: "SET_FORM_ERRORS",
      errors
  ...
  const handlePublish = (0,react.useCallback)(() => EditViewDataManagerProvider_async(undefined, null, function* () {
    const schema = utils_schema(currentContentTypeLayout, {
  ...
  const shouldCheckDZErrors = (0,react.useCallback)((dzName) => {
  ...
  const moveComponentDown = (0,react.useCallback)((dynamicZoneName, currentIndex) => {
  ...
  const moveComponentUp = (0,react.useCallback)((dynamicZoneName, currentIndex) => {
    trackUsageRef.current("changeComponentsOrder");
    dispatch({
  ...
  const removeComponentFromDynamicZone = (0,react.useCallback)((dynamicZoneName, index) => {
...
const SingleTypeFormWrapper = ({ allLayoutData, children, slug }) => {
  ...
  const cleanReceivedData = (0,react.useCallback)((data2) => {
    const cleaned = utils_removePasswordFieldsFromData(data2, allLayoutData.contentType, allLayoutData.components);
  ...
  (0,react.useEffect)(() => {
    const CancelToken = (axios_default()).CancelToken;
    const source = CancelToken.source();
    const fetchData = (source2) => SingleTypeFormWrapper_async(undefined, null, function* () {
      dispatch(getData());
  ...
  const displayErrors = (0,react.useCallback)((err) => {
    ...
    let errorMessage = get_default()(errorPayload, ["message"], "Bad Request");
    if (Array.isArray(errorMessage)) {
      errorMessage = get_default()(errorMessage, ["0", "messages", "0", "id"]);
    }
    if (typeof errorMessage === "string") {
  ...
  const onDelete = (0,react.useCallback)((trackerProperty) => SingleTypeFormWrapper_async(undefined, null, function* () {
    try {
      trackUsageRef.current("willDeleteEntry", trackerProperty);
  ...
  const onPost = (0,react.useCallback)((body, trackerProperty) => SingleTypeFormWrapper_async(undefined, null, function* () {
  ...
  const onPublish = (0,react.useCallback)(() => SingleTypeFormWrapper_async(undefined, null, function* () {
    try {
      trackUsageRef.current("willPublishEntry");
  ...
  const onPut = (0,react.useCallback)((body, trackerProperty) => SingleTypeFormWrapper_async(undefined, null, function* () {
    const endPoint = SingleTypeFormWrapper_utils_getRequestUrl(`${slug}${rawQuery}`);
    try {
      trackUsageRef.current("willEditEntry", trackerProperty);
      dispatch(setStatus("submit-pending"));
  ...
  const onUnpublish = (0,react.useCallback)(() => SingleTypeFormWrapper_async(undefined, null, function* () {
...
var src_utils = __webpack_require__(58836);
...
const Header = ({
  ...
  return /* @__PURE__ */ react.createElement(react.Fragment, null, /* @__PURE__ */ react.createElement(Layout.HeaderLayout, {
    ...
    primaryAction,
    subtitle,
    navigationAction: /* @__PURE__ */ react.createElement(design_system_Link.Link, {
...
const DeleteLink = ({ isCreatingEntry, onDelete, onDeleteSucceeded, trackerProperty }) => {
  ...
  const handleConfirmDelete = () => DeleteLink_async(undefined, null, function* () {
    try {
      ...
      const errorMessage = get_default()(err, "response.payload.message", formatMessage({ id: utils_getTrad("error.record.delete") }));
      setIsModalConfirmButtonLoading(false);
      toggleWarningDelete();
      toggleNotification({ type: "warning", message: errorMessage });
    }
...
const EditView = ({
  ...
  const DataManagementWrapper = (0,react.useMemo)(() => isSingleType ? components_SingleTypeFormWrapper : components_CollectionTypeFormWrapper, [isSingleType]);
  ...
  const formattedContentTypeLayout = (0,react.useMemo)(() => {
    ...
    return utils_createAttributesLayout(currentContentTypeLayoutData.layouts.edit, currentContentTypeLayoutData.attributes);
  ...
  return /* @__PURE__ */ react.createElement(DataManagementWrapper, {
    ...
    }, { number: displayedRelationsLength })), /* @__PURE__ */ react.createElement(Box.Box, {
      paddingTop: 2,
      paddingBottom: 6
    }, /* @__PURE__ */ react.createElement(Divider.Divider, null)), /* @__PURE__ */ react.createElement(Stack.Stack, {
      spacing: 4
...
const EditSettingsView_reducer_reducer = (state = EditSettingsView_reducer_initialState, action) => (0,immer_cjs_production_min["default"])(state, (draftState) => {
  ...
  switch (action.type) {
    ...
    case "MOVE_RELATION": {
      ...
      set_default()(draftState, layoutPathRelations, utils_arrayMoveItem(editRelationLayoutValue, fromIndex, toIndex));
      break;
    }
    case "MOVE_ROW": {
    ...
    case "ON_ADD_FIELD": {
      ...
      const size = getDefaultInputSize(get_default()(newState, ["modifiedData", "attributes", action.name, "type"], ""));
      ...
      if (Array.isArray(rowContentToSet)) {
        set_default()(newList, [listSize > 0 ? listSize - 1 : 0, "rowContent"], [...rowContentToSet, { name: action.name, size }]);
      } else {
        set_default()(newList, [listSize > 0 ? listSize - 1 : 0, "rowContent"], [{ name: action.name, size }]);
      }
    ...
    case "REMOVE_FIELD": {
      ...
      if (row.length === 1 || row.length === 2 && get_default()(row, [1, "name"], "") === "_TEMP_") {
        ...
        set_default()(newState, layoutPathEdit, currentRowFieldList.filter((_, index) => action.rowIndex !== index));
      } else {
        set_default()(newState, [...layoutPathEdit, action.rowIndex, "rowContent"], row.filter((_, index) => index !== action.fieldIndex));
      }
    ...
    case "REMOVE_RELATION": {
      ...
      set_default()(draftState, layoutPathRelations, relationList.filter((_, index) => action.index !== index));
      break;
    }
    case "REORDER_DIFF_ROW": {
      const actualRowContent = get_default()(state, [...layoutPathEdit, action.dragRowIndex, "rowContent"], []);
      const targetRowContent = get_default()(state, [...layoutPathEdit, action.hoverRowIndex, "rowContent"], []);
      const itemToInsert = get_default()(state, [...layoutPathEdit, action.dragRowIndex, "rowContent", action.dragIndex], {});
      const rowContent = [...targetRowContent, itemToInsert];
      let newState = cloneDeep_default()(state);
      set_default()(newState, [...layoutPathEdit, action.dragRowIndex, "rowContent"], actualRowContent.filter((_, index) => action.dragIndex !== index));
      set_default()(newState, [...layoutPathEdit, action.hoverRowIndex, "rowContent"], utils_arrayMoveItem(rowContent, rowContent.length - 1, action.hoverIndex));
      const updatedList = formatLayout(get_default()(newState, layoutPathEdit, []));
      set_default()(draftState, layoutPathEdit, updatedList);
      break;
    }
    case "REORDER_ROW": {
      const newState = cloneDeep_default()(state);
      const rowContent = get_default()(newState, [...layoutPathEdit, action.dragRowIndex, "rowContent"], []);
      set_default()(newState, [...layoutPathEdit, action.dragRowIndex, "rowContent"], utils_arrayMoveItem(rowContent, action.dragIndex, action.hoverIndex));
      const updatedList = formatLayout(get_default()(newState, layoutPathEdit, []));
      set_default()(draftState, layoutPathEdit, updatedList);
      break;
    }
    ...
    case "SUBMIT_META_FORM": {
      set_default()(draftState, ["modifiedData", "metadatas", state.metaToEdit, "edit"], state.metaForm.metadata);
...
const RelationalFields = ({
  ...
  return /* @__PURE__ */ react.createElement(Stack.Stack, {
    ...
    const relationLabel = get_default()(modifiedData, ["metadatas", relationName, "edit", "label"], "");
...
var Enumeration = __webpack_require__(77019);
...
const ModalForm = ({ onMetaChange, onSizeChange }) => {
  ...
  const getSelectedItemSelectOptions = (0,react.useCallback)((formType) => {
  ...
  const metaFields = formToDisplay.map((meta) => {
    ...
    if (formType === "dynamiczone" || formType === "component" && !["label", "description"].includes(meta)) {
      return null;
    }
...
const FormModal = ({ onToggle, onMetaChange, onSizeChange, onSubmit, type }) => {
  ...
  return /* @__PURE__ */ react.createElement(ModalLayout.ModalLayout, {
    ...
    id: utils_getTrad("containers.ListSettingsView.modal-form.edit-label"),
    defaultMessage: "Edit {fieldName}"
  }, { fieldName: upperFirst_default()(selectedField) })))), /* @__PURE__ */ react.createElement(ModalLayout.ModalBody, null, /* @__PURE__ */ react.createElement(Grid.Grid, {
    gap: 4
...
const putCMSettingsEV = (body, slug, isContentTypeView) => {
  return utils/* axiosInstance.put */.be.put(utils_getRequestUrl(isContentTypeView ? `content-types/${slug}/configuration` : `components/${slug}/configuration`), body);
};
...
const EditSettingsView = ({ mainLayout, components, isContentTypeView, slug, updateLayout }) => {
  const [reducerState, dispatch] = (0,react.useReducer)(EditSettingsView_reducer, EditSettingsView_reducer_initialState, () => EditSettingsView_init(EditSettingsView_reducer_initialState, mainLayout, components));
  const [isDraggingSibling, setIsDraggingSibling] = (0,react.useState)(false);
  const { trackUsage } = (0,build.useTracking)();
  const toggleNotification = (0,build.useNotification)();
  const { goBack } = (0,react_router_dom_min.useHistory)();
  ...
  const submitMutation = (0,react_query_lib.useMutation)((body) => {
...
var ListView_constants = __webpack_require__(32174);
...
var ListView_reducer = __webpack_require__(73165);
...
MultipleMedia.propTypes = {
  value: prop_types_default().arrayOf(prop_types_default().shape({
    alternativeText: (prop_types_default()).string,
    ext: (prop_types_default()).string.isRequired,
    formats: (prop_types_default()).object,
    mime: (prop_types_default()).string.isRequired,
...
const TypographyMaxWidth = (0,styled_components_browser_cjs["default"])(Typography.Typography)`
  max-width: 500px;
`;
const fetchRelation = (endPoint, notifyStatus) => RelationMultiple_async(undefined, null, function* () {
  const {
...
const RelationMultiple = ({ fieldSchema, metadatas, queryInfos, name, rowId, value }) => {
  ...
  const { data, status } = (0,react_query_lib.useQuery)([fieldSchema.targetModel, rowId], () => fetchRelation(requestURL, notify), {
    enabled: isOpen,
    staleTime: 0
  });
...
const TableRows = ({
  ...
  return /* @__PURE__ */ react.createElement(Table.Tbody, null, rows.map((data, index) => {
    ...
    return /* @__PURE__ */ react.createElement(Table.Tr, TableRows_spreadValues({
      ...
      label: formatMessage({ id: "app.component.table.edit", defaultMessage: "Edit {target}" }, { target: itemLineText }),
      noBorder: true,
      icon: /* @__PURE__ */ react.createElement((Pencil_default()), null)
      ...
      label: formatMessage({ id: "global.delete-target", defaultMessage: "Delete {target}" }, { target: itemLineText }),
      noBorder: true,
      icon: /* @__PURE__ */ react.createElement((Trash_default()), null)
    })))));
  }));
...
const DynamicTable = ({
  ...
  const { formatMessage } = (0,react_intl.useIntl)();
  const displayedHeaders = (0,lib/* useSelector */.v9)(selectDisplayedHeaders);
...
const FieldPicker = ({ layout }) => {
  ...
  return /* @__PURE__ */ react.createElement(Box.Box, {
    ...
    value: values,
    onChange: handleChange,
    customizeContent: (values2) => `${values2.length} currently selected`,
    multi: true,
    size: "S"
...
Filters.propTypes = {
  displayedFilters: prop_types_default().arrayOf(prop_types_default().shape({
    name: (prop_types_default()).string.isRequired,
...
function ListView({
  ...
  const fetchData = (0,react.useCallback)((endPoint, source) => ListView_async(this, null, function* () {
    getData2();
    try {
  ...
  const handleConfirmDeleteAllData = (0,react.useCallback)((ids) => ListView_async(this, null, function* () {
    try {
  ...
  const handleConfirmDeleteData = (0,react.useCallback)((idToDelete) => ListView_async(this, null, function* () {
    try {
  ...
  return /* @__PURE__ */ react.createElement(Main.Main, {
    ...
    startActions: /* @__PURE__ */ react.createElement(react.Fragment, null, isSearchable && /* @__PURE__ */ react.createElement(build.SearchURLQuery, {
      label: formatMessage({ id: "app.component.search.label", defaultMessage: "Search for {target}" }, { target: headerLayoutTitle }),
...
const withConnect = (0,lib/* connect */.$j)(mapStateToProps, mapDispatchToProps);
...
/* harmony default export */ const ListViewLayoutManager_Permissions = ((0,react.memo)(Permissions_Permissions, (prev, next) => {
  ...
  const propNamesThatHaveChanged = Object.keys(differenceBetweenRerenders).filter((propName) => propName !== "state");
...
SortDisplayedFields.propTypes = {
  ...
  metadatas: prop_types.PropTypes.objectOf(prop_types.PropTypes.shape({
...
const EditFieldForm = ({
  ...
  return /* @__PURE__ */ react.createElement(ModalLayout.ModalLayout, {
    ...
    id: utils_getTrad("containers.ListSettingsView.modal-form.edit-label"),
    defaultMessage: "Edit {fieldName}"
  }, { fieldName: upperFirst_default()(fieldToEdit) })))), /* @__PURE__ */ react.createElement(ModalLayout.ModalBody, null, /* @__PURE__ */ react.createElement(Grid.Grid, {
    gap: 4
  }, /* @__PURE__ */ react.createElement(Grid.GridItem, {
    s: 12,
    col: 6
...
EditFieldForm.propTypes = {
  attributes: prop_types_default().objectOf(prop_types_default().shape({
...
const ListSettingsView_reducer_reducer = (state = ListSettingsView_reducer_initialState, action) => (0,immer_cjs_production_min["default"])(state, (draftState) => {
  ...
  switch (action.type) {
    ...
    case "MOVE_FIELD": {
      ...
      set_default()(draftState, layoutFieldListPath, utils_arrayMoveItem(layoutFieldList, originalIndex, atIndex));
      break;
    }
    case "ON_CHANGE": {
    ...
    case "REMOVE_FIELD": {
      const layoutFieldList = get_default()(state, layoutFieldListPath, []);
      set_default()(draftState, layoutFieldListPath, layoutFieldList.filter((_, index) => action.index !== index));
      break;
    }
    case "SET_FIELD_TO_EDIT": {
      const { fieldToEdit } = action;
      draftState.fieldToEdit = fieldToEdit;
      draftState.fieldForm.label = get_default()(draftState, ["modifiedData", "metadatas", fieldToEdit, "list", "label"], "");
      draftState.fieldForm.sortable = get_default()(draftState, ["modifiedData", "metadatas", fieldToEdit, "list", "sortable"], "");
      break;
    }
    case "UNSET_FIELD_TO_EDIT": {
...
const ListSettingsView = ({ layout, slug }) => {
  ...
  const [reducerState, dispatch] = (0,react.useReducer)(ListSettingsView_reducer, ListSettingsView_reducer_initialState, () => ListSettingsView_init(ListSettingsView_reducer_initialState, layout));
  ...
  const goBackUrl = () => {
    ...
    const goBackSearch = `${(0,qs_lib.stringify)({
      ...
      pageSize,
      sort
    }, { encode: false })}${pluginsQueryParams ? `&${pluginsQueryParams}` : ""}`;
  ...
  const handleConfirm = () => ListSettingsView_async(undefined, null, function* () {
...
var crudReducer_reducer = __webpack_require__(68014);
...
const ComponentSettingsView = () => {
  ...
  (0,react.useEffect)(() => {
    const CancelToken = (axios_default()).CancelToken;
    const source = CancelToken.source();
    const fetchData = (source2) => ComponentSetttingsView_async(undefined, null, function* () {
      try {
        dispatch(getData());
        const {
          data: { data }
...
var match_sorter_cjs = __webpack_require__(46517);
...
const LeftMenu = () => {
  ...
  const { collectionTypeLinks, singleTypeLinks } = (0,lib/* useSelector */.v9)((state) => modelLinksSelector(state), lib/* shallowEqual */.wU);
  ...
  const menu = [
    {
      ...
      searchable: true,
      links: sortBy_default()(matchByTitle(intlCollectionTypeLinks, search), (object) => object.title.toLowerCase())
    },
    {
      id: "singleTypes",
      title: {
      ...
      searchable: true,
      links: sortBy_default()(matchByTitle(intlSingleTypeLinks, search), (object) => object.title.toLowerCase())
    }
  ];
  const handleClear = () => {
    setSearch("");
  ...
  return /* @__PURE__ */ react.createElement(SubNav.SubNav, {
    ...
    const label2 = formatMessage({ id: section.title.id, defaultMessage: section.title.defaultMessage }, section.title.values);
...
var App_constants = __webpack_require__(27904);
...
const generateModelsLinks = (models, modelsConfigurations) => {
  ...
  return {
    collectionTypesSectionLinks: generateLinks((0,lodash.get)(collectionTypes, "links", []), "collectionTypes", modelsConfigurations),
...
const getContentTypeLinks = (models, userPermissions, toggleNotification) => getContentTypeLinks_async(undefined, null, function* () {
  try {
    const {
      data: { data: contentTypeConfigurations }
    } = yield utils/* axiosInstance.get */.be.get(utils_getRequestUrl("content-types-settings"));
    const { collectionTypesSectionLinks, singleTypesSectionLinks } = utils_generateModelsLinks(models, contentTypeConfigurations);
    const ctLinksPermissionsPromises = utils_checkPermissions(userPermissions, collectionTypesSectionLinks);
    const ctLinksPermissions = yield Promise.all(ctLinksPermissionsPromises);
    const authorizedCtLinks = collectionTypesSectionLinks.filter((_, index) => ctLinksPermissions[index]);
    ...
    const authorizedStLinks = singleTypesSectionLinks.filter((_, index) => stLinksPermissions[index]);
...
const useModels = () => {
  const dispatch = (0,lib/* useDispatch */.I0)();
  const toggleNotification = (0,build.useNotification)();
  const state = (0,lib/* useSelector */.v9)(selectAppDomain());
  ...
  const { notifyStatus } = (0,LiveRegions.useNotifyAT)();
  const { formatMessage } = (0,react_intl.useIntl)();
  const fetchData = () => useModels_async(undefined, null, function* () {
    ...
    try {
      ...
      ] = yield Promise.all(["components", "content-types"].map((endPoint) => utils/* axiosInstance.get */.be.get(utils_getRequestUrl(endPoint), { cancelToken: source.token })));
      notifyStatus(formatMessage({
        id: utils_getTrad("App.schemas.data-loaded"),
        defaultMessage: "The schemas have been successfully loaded."
      }));
      const { authorizedCtLinks, authorizedStLinks } = yield utils_getContentTypeLinks(models, allPermissions, toggleNotification);
...
const App = () => {
  ...
  const authorisedModels = sortBy_default()([...collectionTypeLinks, ...singleTypeLinks], (model) => model.title.toLowerCase());
  const { pathname } = (0,react_router_dom_min.useLocation)();
  const { formatMessage } = (0,react_intl.useIntl)();
