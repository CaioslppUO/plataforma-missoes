...
const Icon = styled_components_browser_cjs["default"].svg(({ theme, color }) => `
  width: ${12 / 16}rem;
  height: ${12 / 16}rem;
...
const schema = yup_lib/* object */.Ry().shape({
  ...
  headers: yup_lib/* lazy */.Vo((array) => {
    ...
    return baseSchema.of(yup_lib/* object */.Ry().shape({
...
const EditView = () => {
  ...
  const fetchWebhook = (0,react.useCallback)((id2) => __async(undefined, null, function* () {
    const [err, { data: data2 }] = yield (0,build.to)((0,build.request)(`/admin/webhooks/${id2}`, {
      method: "GET"
    }));
    if (err) {
      toggleNotification({
  ...
  const createWebhookMutation = (0,lib.useMutation)((body) => (0,build.request)("/admin/webhooks", {
    method: "POST",
    body
  }));
  const updateWebhookMutation = (0,lib.useMutation)(({ id: id2, body }) => (0,build.request)(`/admin/webhooks/${id2}`, {
    method: "PUT",
    body
  }));
  const handleSubmit = (data2) => __async(undefined, null, function* () {
    if (isCreating) {
      lockApp();
      ...
      lockApp();
      updateWebhookMutation.mutate({ id, body: formatData(data2) }, {
        onSuccess: () => {
  ...
  const isDraftAndPublishEvents = (0,react.useMemo)(() => collectionTypes.some((ct) => ct.options.draftAndPublish === true), [collectionTypes]);
...
/* harmony import */ var _permissions__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(92699);
/* harmony import */ var _EditView__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(98776);
