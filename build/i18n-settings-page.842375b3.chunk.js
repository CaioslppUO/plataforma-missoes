...
var constants = __webpack_require__(70437);
...
const editLocale = (id, payload, toggleNotification) => __async(undefined, null, function* () {
  try {
...
const useEditLocale = () => {
  ...
  const modifyLocale = (id, payload) => __async(undefined, null, function* () {
    setLoading(true);
...
const ModalEdit = ({ locale, onClose }) => {
  ...
  const { formatMessage } = (0,react_intl.useIntl)();
  const handleSubmit = (_0) => ModalEdit_async(undefined, [_0], function* ({ displayName, isDefault }) {
...
const deleteLocale = (id, toggleNotification) => useDeleteLocale_async(undefined, null, function* () {
  try {
    const data = yield (0,build.request)(`/i18n/locales/${id}`, {
      method: "DELETE"
    });
...
const useDeleteLocale = () => {
  ...
  const dispatch = (0,react_redux_lib/* useDispatch */.I0)();
  const toggleNotification = (0,build.useNotification)();
  const removeLocale = (id) => useDeleteLocale_async(undefined, null, function* () {
    setLoading(true);
...
const addLocale = (_0, _1) => useAddLocale_async(undefined, [_0, _1], function* ({ code, name, isDefault }, toggleNotification) {
...
const useAddLocale = () => {
  ...
  const dispatch = (0,react_redux_lib/* useDispatch */.I0)();
  const toggleNotification = (0,build.useNotification)();
  const persistLocale = (locale) => useAddLocale_async(undefined, null, function* () {
    setLoading(true);
    try {
...
const fetchDefaultLocalesList = (toggleNotification) => useDefaultLocales_async(undefined, null, function* () {
  try {
...
const useDefaultLocales = () => {
  ...
  const toggleNotification = (0,build.useNotification)();
  const { isLoading, data } = (0,react_query_lib.useQuery)("default-locales", () => fetchDefaultLocalesList(toggleNotification).then((data2) => {
...
const BaseForm_BaseForm = () => {
  ...
  const handleLocaleChange = (0,react.useCallback)((nextLocale) => {
...
const ModalCreate = ({ onClose }) => {
  ...
  const { formatMessage } = (0,react_intl.useIntl)();
  const { refetchPermissions } = (0,build.useRBACProvider)();
  const handleLocaleAdd = (values) => ModalCreate_async(undefined, null, function* () {
    yield addLocale({
      code: values.code,
...
var permissions = __webpack_require__(97473);
