...
const fetchUser = () => __async(undefined, null, function* () {
  const { data } = yield utils/* axiosInstance.get */.be.get("/admin/users/me");
  return data.data;
});
const putUser = (body) => __async(undefined, null, function* () {
...
const ProfilePage = () => {
  ...
  const submitMutation = (0,lib.useMutation)((body) => putUser(body), {
    onSuccess: (data2) => ProfilePage_async(undefined, null, function* () {
      yield queryClient.invalidateQueries("user");
      build.auth.setUserInfo(pick_default()(data2, ["email", "firstname", "lastname", "username", "preferedLanguage"]));
  ...
  const handleSubmit = (_0, _1) => ProfilePage_async(undefined, [_0, _1], function* (body, { setErrors }) {
    lockApp();
    const username = body.username || null;
    submitMutation.mutate(ProfilePage_spreadProps(ProfilePage_spreadValues({}, body), { username }), {
      onError: (error) => {
        var _a;
  ...
  const themesToDisplay = Object.keys(allApplicationThemes).filter((themeName) => allApplicationThemes[themeName]);
  return /* @__PURE__ */ react.createElement(Main.Main, {
