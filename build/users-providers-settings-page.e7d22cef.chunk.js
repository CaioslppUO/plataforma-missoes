...
const fetchData = (toggleNotification) => __async(undefined, null, function* () {
  try {
...
const createProvidersArray = (data) => {
  return (0,lodash.sortBy)(Object.keys(data).reduce((acc, current) => {
...
const Input = ({
  ...
  const label = formatMessage({ id: intlLabel.id, defaultMessage: intlLabel.defaultMessage }, __spreadValues({ provider: providerToEditName }, intlLabel.values));
  const hint = description ? formatMessage({ id: description.id, defaultMessage: description.defaultMessage }, __spreadValues({ provider: providerToEditName }, description.values)) : "";
  if (type === "bool") {
  ...
  const formattedPlaceholder = placeholder ? formatMessage({ id: placeholder.id, defaultMessage: placeholder.defaultMessage }, __spreadValues({}, placeholder.values)) : "";
...
const ProvidersPage = () => {
  ...
  const { isLoading: isLoadingForData, data: modifiedData, isFetching } = (0,lib.useQuery)("get-providers", () => fetchData(toggleNotification), {
    onSuccess: () => {
  ...
  const submitMutation = (0,lib.useMutation)(putProvider, {
    onSuccess: () => Providers_async(undefined, null, function* () {
  ...
  const handleSubmit = (values) => Providers_async(undefined, null, function* () {
    setIsSubmiting(true);
    lockApp();
